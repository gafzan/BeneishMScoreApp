...
ACCOUNTING_ITEMS_CONFIG = {
    AccountingItemKeys.REVENUES: {
        ...
        'seeking_alpha_statement': 'income-statement'
    },
...
'seeking_alpha_statement': 'cash-flow-statement'
    },
...
'seeking_alpha_statement': 'balance-sheet'
    ...
    def on_enter():
        ...
        if st.session_state.identity_input:
            if check_identity(id_input=st.session_state.identity_input):
                ...
                st.session_state.identity_error = "❌ Invalid format. Please use: 'Name contact@domain.com'"
                st.session_state.identity_submitted = False
...
def create_input_form():
    ...
    with st.form("financial_data_form"):
        ...
        for index_item in default_df.index:
            cols = st.columns([1] + [1] * num_data_cols)
            with cols[0]:
                if 'ticker_input' in st.session_state and len(st.session_state.ticker_input):
                    url = f'https://seekingalpha.com/symbol/{st.session_state.ticker_input.upper()}/{ACCOUNTING_ITEMS_LONG_NAME_CONFIG[index_item]['seeking_alpha_statement']}'
                    st.markdown(f"[**{index_item}**](%s)" % url, help=ACCOUNTING_ITEMS_LONG_NAME_CONFIG[index_item]['help'])
                else:
