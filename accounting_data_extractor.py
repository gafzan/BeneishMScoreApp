"""accounting_data_extractor.py"""

import pandas as pd
import re
from typing import Dict, Literal, Union, List as TypingList
from functools import reduce
import operator

from accounting_item_filter_config import (REVENUE_ITEMS_FILTER, COGS_ITEMS_FILTER, DEPRECIATION_AMORTIZATION_FILTER,
                                           SALES_GENERAL_ADMIN_FILTER, ACCOUNTS_RECEIVABLE_FILTER, CURRENT_ASSET_FILTER,
                                           CURRENT_LIABILITIES_FILTER, TOTAL_ASSETS_FILTER,
                                           PROPERTY_PLANT_EQUIPMENT_FILTER, LONG_TERM_DEBT_FILTER, NET_INCOME_FILTER,
...
class AccountingDataExtractor:
    ...
    def _partial_match(
        ...
        # Loop through each mach by columns (label or concept)
        ...
        # Combine results across columns (AND between columns)
        include_mask = reduce(operator.and_, include_masks) if include_masks else pd.Series(False, index=self.df.index)
        ...
        if not matches.empty:
            logger.info(f"Found {matches.shape[0]} partial matches across '%s' from filter config." % "' and '".join(match_by))
    ...
    def extract_accounting_items(self, search_term: str = None, filter_config: Dict[str, TypingList[str]] = None,
        ...
        if search_term:
            ...
            match_method_configs = [
                ...
                (self._partial_match, {'match_by': ['concept', 'label'], 'filter_config': filter_config}),  # Level 3: Partial match with concept AND label with excl.
                (self._partial_match, {'match_by': 'concept', 'filter_config': filter_config}),  # Level 4: Partial concept match with exclusions
                (self._partial_match, {'match_by': 'label', 'filter_config': filter_config}),  # Level 5: Partial label match from common items
            ]
        ...
        # If there are multiple matches, pick the one that has 'style' set to Total if there is one
        if filter_by_total and matches.shape[0] > 1 and 'total' in matches['style'].str.lower().to_list():
            logger.info(f"Filter based on 'Total' (rows before = {matches.shape[0]})")
            matches = matches[matches['style'].str.lower() == 'total'].copy()
            logger.info(f"Filter based on 'Total' (rows after = {matches.shape[0]})")
        return matches

    def extract_revenue_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=REVENUE_ITEMS_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_cogs_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=COGS_ITEMS_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_depreciation_amortization_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=DEPRECIATION_AMORTIZATION_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_sales_general_admin_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=SALES_GENERAL_ADMIN_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_accounts_receivable_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=ACCOUNTS_RECEIVABLE_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_current_asset_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=CURRENT_ASSET_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_current_liabilities_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=CURRENT_LIABILITIES_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_total_assets_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=TOTAL_ASSETS_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_property_plant_equipment_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=PROPERTY_PLANT_EQUIPMENT_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_long_term_debt_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=LONG_TERM_DEBT_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_net_income_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
            filter_config=NET_INCOME_FILTER,
            filter_by_total=filter_by_total
        )

    def extract_cash_from_operations_items(self, filter_by_total: bool = True):
        return self.extract_accounting_items(
